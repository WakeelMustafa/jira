module Jira
  class FetchJiraIssuesJob < ApplicationJob
    ...
    def perform(current_user, project_id, id)
      ...
      if result[:error].present?
        ...
        @issues.each do |issue_data|
          ...
          if jira_user_data
            jira_user = JiraUser.find_or_create_by(account_id: jira_user_data[:account_id]) do |user|
              user.display_name = jira_user_data[:display_name]
              user.avatar_url = jira_user_data[:avatarUrls]["48x48"] # Or whichever size you prefer
            end
          end
